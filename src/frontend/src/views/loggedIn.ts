...
export const renderLoggedIn = (
	...
	authClient: AuthClient | iiAuth.AuthClient
) => {
	render(content(), document.getElementById('pageContent') as HTMLElement);

	(document.getElementById('whoamiButton') as HTMLButtonElement).onclick = async () => {
		try {
	...
	(document.getElementById('logout') as HTMLButtonElement).onclick = async () => {
